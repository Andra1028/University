...
void ConsoleGUIProgram::reloadDynamicButtons() {
	...
	set<string> gen = this->getGen(Cosbooks);

	for (string genre : genres) {
		QPushButton* btn = new QPushButton{ QString::fromStdString(gen) };
		lyBtnDynamic->addWidget(btn);
		int howMany = howManyWithGenre(Cosbooks, gen);
		QObject::connect(btn, &QPushButton::clicked, [=]() {
			QMessageBox::information(this, "Info", QString::fromStdString("Carti cu genul " + gen + " : " + to_string(howMany)));
...
void ConsoleGUIProgram::connectSignalsSlotsProg() {
	...
	QObject::connect(btnDrawWindow, &QPushButton::clicked, [&]() {
		auto bookdraw = new SongDraw(srv.getAllCos());
		bookdraw->show();
		});
